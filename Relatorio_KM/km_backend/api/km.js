...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === 'POST') {
      const doc = req.body;
      // validação básica
      if (!doc.data || !doc.local || typeof doc.kmSaida !== 'number' || typeof doc.kmChegada !== 'number') {
        return res.status(400).json({ error: 'Campos inválidos. data, local, kmSaida (number) e kmChegada (number) são obrigatórios.' });
      }
