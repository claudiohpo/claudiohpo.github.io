...
a.href = url; a.download = 'relatorio_km.csv'; a.click(); URL.revokeObjectURL(url);
} catch (e) {
const stored = JSON.parse(localStorage.getItem('km_pending') || '[]');
if (stored.length === 0) { alert('Nenhum dado disponível para relatório local.'); return; }
...
const blob = new Blob([csv], {type: 'text/csv'});
const url = URL.createObjectURL(blob);
const a = document.createElement('a'); a.href = url; a.download = 'relatorio_km_local.csv'; a.click(); URL.revokeObjectURL(url);
}
});